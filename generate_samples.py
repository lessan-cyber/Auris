...
def create_variants(input_a, output_dir, input_b=None):
    ...
    run_ffmpeg([
        "ffmpeg", "-y", "-i", base_file,
        "-f", "lavfi", "-i", "anoisesrc=a=0.2:c=white",
        "-filter_complex", "[0:a][1:a]amix=inputs=2:duration=first",
        f"{output_dir}/var_noise.mp3",
    ])
    ...
    run_ffmpeg([
        "ffmpeg", "-y", "-i", base_file,
        "-af", "highpass=f=300,lowpass=f=3000,volume=1.5",
        f"{output_dir}/var_phone.mp3",
    ])

    # 6. Variant: Worst case (noise + speed + eq + echo)
    print("Generating Worst variant...")
    run_ffmpeg([
        "ffmpeg", "-y", "-i", base_file,
        "-f", "lavfi", "-i", "anoisesrc=a=0.1:c=white",
        "-filter_complex",
        "[1:a]lowpass=f=3000,highpass=f=300,volume=1.5[noise];"
        "[0:a]atempo=1.10[music];"
        "[music][noise]amix=inputs=2:duration=first:dropout_transition=0[mixed];"
        "[mixed]aecho=0.8:0.88:60:0.4,highpass=f=200,lowpass=f=3500,volume=0.8",
