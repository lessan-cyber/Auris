...
export function Library() {
    ...
    // Use the upload notification context
    const uploadNotificationContext = useContext<UploadNotificationContextType | undefined>(UploadNotificationContext);
    const setUploadNotifications = uploadNotificationContext?.setUploadNotifications || (() => {});
    ...
    // Polling for upload progress notifications
    useEffect(() => {
        ...
        const pollInterval = setInterval(async () => {
            try {
                ...
                setUploadNotifications(prev => {
                    const existingIndex = prev.findIndex(n => n.track_id === trackId);
                    const newNotification = { track_id: trackId, status: track.status, message };
                    
                    if (existingIndex >= 0) {
                        // Update existing notification
                        return prev.map((n, i) => i === existingIndex ? newNotification : n);
                    } else {
                ...
                // If track is ready or error, stop polling and clear storage
                if (track.status === "ready" || track.status === "error") {
                    clearInterval(pollInterval);
                    localStorage.removeItem("recentUploadTrackId");
                    // Refresh track list to show the new track
                    queryClient.invalidateQueries({ queryKey: ["tracks"] });
                }
            } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-4 py-12 bg-background transition-colors duration-300">
            ...
            {hasTracks ? (
                ...
                <div className="flex flex-col items-center justify-center py-24 text-muted-foreground">
                    <SoundMin className="w-12 h-12 mb-4 opacity-20" strokeWidth={1} />
