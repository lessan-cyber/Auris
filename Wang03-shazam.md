...
unknown    sample    and    tracks    within    the    database.
Insufficient   entropy   leads   to   excessive   and   spurious
...
Finding   the   correct   registration   offset   directly   from
...
processed   is   approximately   equal   to   the   density   of
...
out   of   size   F=10,   then   the   number   of   hashes   is
approximately   equal   to   F=10   times   the   number   of
...
speedup  is  a  factor  of  about  1000000/F
## 2
,  or  about  10000,
...
approximately p
## 2
.  This reduction in hash survivability is a
tradeoff   against   the   tremendous   amount   of   speedup
...
is    p*[1-(1-p)
## F
].    For  reasonably  large  values  of  F,  e.g.
...
corresponding    times    of    matching    features    between
...
where  t
k
’  is  the  time  coordinate  of  the  feature  in  the
matching  (clean)  database  soundfile  and  t
k
is  the  time
coordinate   of   the   corresponding   feature   in   the   sample
soundfile to be identified.  For each (t
k
## ’,t
k
) coordinate in the
...
k
## .
Then we calculate a histogram of these  δt
k
values and scan
for a peak. This may be done by sorting the set of δt
k
values
...
match   in   less   than   10   milliseconds,   with   a   likely
optimization  goal  reaching  down  to  1  millisecond  per
query.
3.3    Specificity and False Positives
The    algorithm    was    designed    specifically    to    target
...
false    positives,    and    thus    the    maximum    allowable
